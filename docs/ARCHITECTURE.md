...
| 调度层 | Dispatcher (`dispatcher.py`), SecurityShell (`constraints.py`), SubmitClassifier/TaskPromptCompiler (`strategy.py`) | 状态机驱动执行，安全壳验证，stagnation/submit 策略逻辑 | schedule() → planner.plan() → SecurityShell.validate() → runtime.run_task() + should_abandon/stage_after_program |
