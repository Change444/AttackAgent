...
│  │  - validate() - 安全约束验证                       │  │
│  │  - _check_parameter_scope() - 参数 URL scope 验证 🆕 │  │
│  │  - 在 runtime 执行前验证                           │  │
│  └────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────┘
                           ↓
...
class CompetitionPlatform:
    def __init__(self,
        ...
        """初始化平台，agent_config.security 驱动安全约束，agent_config.model.observation_summary_budget_chars 驱动 ObservationSummarizer 🆕"""
...
class LightweightSecurityShell:
    ...
    def _check_parameter_scope(self, bundle: TaskBundle) -> list[ConstraintViolation]:
        """验证 step.parameters 中 URL 类参数 (url, login_url, location) 是否匹配 allowed_hostpatterns 🆕"""
```
...
**违规等级：**
- **critical**：阻止执行（包括参数中含外部 URL）
- **warning**：记录但不阻止

**参数 scope 验证 🆕：**

`_URL_PARAM_KEYS` 映射原语到 URL 类参数键名：
```python
_URL_PARAM_KEYS: dict[str, list[str]] = {
    "http-request": ["url"],
    "session-materialize": ["login_url"],
    "browser-inspect": ["url"],
    "artifact-scan": ["location"],
}
```

`_check_parameter_scope()` 检查每个步骤的 `step.parameters` 中 URL 类参数值是否匹配 `allowed_hostpatterns`。不匹配时产生 critical 级违规，阻止执行。
...
`http-request` 族 observation_gate 步骤默认含 `method: "GET"`；`session-materialize` 族 action_template 步骤默认含 `method: "POST"`。这些默认值在有 metadata 时被 metadata 覆盖。
...
**LLM payload 增强 🆕：** `choose_program()` 发送给模型的 payload 现包含 `observation_summaries` 和每候选的 `step_parameters`。

#### ReasoningContext 🆕增强

`ReasoningContext` 新增 `observation_summaries: list[str]` 字段，供 LLMReasoner 和 APGPlanner 注入观测内容摘要。
...
class ConstraintAwareReasoner:
    ...
    def _extract_current_state(self, record: ProjectRecord) -> str:
        """提取当前状态 — 输出实际观测内容 + world_state highlights 🆕"""
```
...
`_PRIMITIVE_PARAM_KEYS` 定义各原语允许的参数键，`_parse_plan_response()` 中未知键被剥离：
```python
_PRIMITIVE_PARAM_KEYS: dict[str, set[str]] = {
    "http-request": {"url", "method", "headers", "body", "body_type", "content_type"},
    "browser-inspect": {"url"},
    "session-materialize": {"login_url", "method", "username_field", "password_field", "username", "password", "body"},
    "structured-parse": {"parse_source", "format", "extract_fields"},
    "diff-compare": {"baseline_observation_id", "variant_observation_id"},
    "artifact-scan": {"location"},
    "binary-inspect": {"location"},
...
- 所有 9 个原始动作均支持真实执行路径
- 无配置时自动回退到元数据消费路径
- **step.parameters 参数化：metadata 缺失时从 parameters 构建 spec；metadata 存在时 parameters 覆盖默认值 🆕**
...
`_step_param_overrides()` helper 从 `step.parameters` 提取参数覆盖（排除 `required_tags`）。所有 5 个 `_resolve_*_specs()` 函数现在支持双模式：
- **metadata 缺失但 step.parameters 有 url/path/login_url** — 从 parameters 构建 spec（替代 metadata）
- **metadata 存在** — `step.parameters` 覆盖 metadata 默认值
...
| 原始动作 | 真实执行路径 | 触发方式 | 参数化 🆕 | 回退路径 |
|----------|-------------|----------|-----------|----------|
| `http-request` | HTTP GET/POST/PUT/DELETE，cookie 持久化，重定向跟随 | `metadata.http_request` | `step.parameters.url/method/headers/body` 覆盖/替代 metadata | `_consume_metadata` |
| `browser-inspect` | HTTP 抓取 + HTMLParser 解析，会话共享 | `metadata.browser_inspect` | `step.parameters.url` 覆盖/替代 metadata | `_consume_metadata` |
| `artifact-scan` | 本地文件扫描 + HTTP 下载 + zip/tar 解压 | `metadata.artifact_scan` 或 `step.parameters.location` | `step.parameters.location` 覆盖/替代 metadata | `_consume_metadata` |
| `binary-inspect` | ASCII/UTF-8/UTF-16LE 字串提取 + ELF/PE 头解析 | `metadata.binary_inspect` 或 `step.parameters.location` | `step.parameters.location` 覆盖/替代 metadata | `_consume_metadata` |
| `code-sandbox` | AST 安全验证的 Python 执行 | `step.instruction` 含 Python 代码 | 无参数化 | `_consume_metadata` |
| `session-materialize` | HTTP POST 登录 + cookie 持久化 | `metadata.session_materialize` | `step.parameters.login_url/method/username/password` 覆盖/替代 metadata | `_consume_metadata` |
| `structured-parse` | JSON/HTML/headers 解析 + 字段提取 | `step.parameters.parse_source/format/extract_fields` | `step.parameters` 激活 + 提供解析源 | `_consume_metadata` |
| `diff-compare` | difflib 序列对比 + 变更统计 | `step.parameters.baseline_observation_id/variant_observation_id` | `step.parameters` 激活 + 指定对比源 | `_consume_metadata` |
| `extract-candidate` | 多模式正则提取 + completed_observations 搜索 | `step.parameters.patterns` 或 `challenge.flag_pattern` | `step.parameters.patterns` 覆盖默认 pattern | `_consume_metadata` |
...
#### 3.4.5 ObservationSummarizer 🆕

**文件位置：** `attack_agent/observation_summarizer.py`

**职责：**
- 将观测 payload 压缩为有限长度文本供 LLM prompt
- 提供类型特定的摘要器（kind-specific summarizers）
- 确保总摘要不超过预算字符数

**核心接口：**
```python
class ObservationSummarizer:
    def __init__(self, config: ObservationSummarizerConfig | None = None):
        """初始化，config 控制总字符预算"""

    def summarize_observations(self, observations: dict[str, Observation]) -> list[str]:
        """为所有观测生成摘要列表"""

    def summarize_single(self, obs: Observation) -> str:
        """单观测摘要，kind-specific 处理"""
```
...
- `browser-page` — title, forms, links, scripts snippet
- `session-materialized` — cookies/tokens obtained
- `parsed-data` — extracted fields summary
- `diff-result` — change statistics
- `artifact-file` — filename, size, key content snippet
- `binary-strings` — string count, notable strings
- `sandbox-output` — output/result summary

**集成位置：**
- `CompetitionPlatform` 从 `agent_config.model.observation_summary_budget_chars` 构建，共享到 `StateGraphService`、`APGPlanner`、`ConstraintAwareReasoner`
- `APGPlanner.plan()` 注入观测摘要到 EpisodeMemory query 和 ReasoningContext
- `ConstraintAwareReasoner._extract_current_state()` 输出观测内容 + world_state highlights
- `StateGraphService.build_episode_entry()` enrich feature_text 和 summary
...
class StateGraphService:
    def __init__(self, observation_summarizer: ObservationSummarizer | None = None):  # 🆕
        """初始化状态图服务，observation_summarizer 用于 enrich build_episode_entry"""
    ...
    def build_episode_entry(self, project_id: str, summarizer: ObservationSummarizer | None = None) -> EpisodeEntry:  # 🆕
        """构建案例条目，summarizer enrich feature_text 和 summary"""
...
@dataclass
class ModelConfig:  # 🆕
    provider: str = "heuristic"           # heuristic / openai / anthropic
    model_name: str = ""                  # 模型标识
    observation_summary_budget_chars: int = 2000  # ObservationSummarizer 总字符预算
...
- [x] ObservationSummarizer — 新模块，将观测 payload 总结为有限长度文本供 LLM prompt ✅
- [x] Primitive 参数化 — `_resolve_*_specs()` 接受 `step.parameters` 作为替代/覆盖源 ✅
- [x] ConstraintAwareReasoner enrichment — `_extract_current_state()` 输出实际内容而非计数 ✅
- [x] ReasoningContext + APGPlanner — 加入 `observation_summaries` 字段，注入观测内容摘要 ✅
...
#### LLM 反馈闭环 + 原语参数化验收 🆕
- [x] ObservationSummarizer 各观测类型摘要正常
- [x] ObservationSummarizer 总字符预算截断正常
- [x] step.parameters 覆盖 metadata 默认值正常
- [x] step.parameters 替代缺失 metadata 正常
- [x] _step_param_overrides() 排除 required_tags 正常
- [x] ConstraintAwareReasoner._extract_current_state() 输出实际内容正常
- [x] ReasoningContext.observation_summaries 注入正常
- [x] 安全壳 _check_parameter_scope() 验证 URL scope 正常
...
| ObservationSummarizer | 观测摘要器，将观测 payload 压缩为有限长度文本供 LLM | 🆕 |
| ObservationSummarizerConfig | 观测摘要配置，max_total_chars/max_single_obs_chars 预算 | 🆕 |
| _step_param_overrides | 参数覆盖 helper，从 step.parameters 提取参数（排除 required_tags） | 🆕 |
| _check_parameter_scope | 安全壳参数 URL scope 验证，外部 URL 产生 critical 违规 | 🆕 |
| _URL_PARAM_KEYS | 原语到 URL 类参数键名映射，用于 scope 验证 | 🆕 |
