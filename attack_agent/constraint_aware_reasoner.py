...
PRIMITIVE_DESCRIPTIONS: dict[str, str] = {
    ...
    "session-materialize": "会话物化(登录)。参数: login_url, method, form_fields, username, password, headers, json, content_type, csrf_token, csrf_field, csrf_source",
    "artifact-scan": "扫描文件内容(含ZIP/tar内容提取)。参数: path, url, location, preview_bytes, max_depth, max_members",
    "binary-inspect": "检查二进制文件。参数: path, url, location, min_length, max_strings",
    "code-sandbox": "在沙盒中执行代码（支持class/with/raise/try，可导入hashlib/base64/struct/binascii/itertools/collections/math/re/json/zlib/csv）。参数: program_fragment, inputs",
...
_PRIMITIVE_PARAM_KEYS: dict[str, set[str]] = {
    ...
    "session-materialize": {"login_url", "method", "form_fields", "username", "password", "headers", "json", "content_type", "csrf_token", "csrf_field", "csrf_source", "timeout", "id"},
    "artifact-scan": {"path", "url", "location", "preview_bytes", "max_depth", "max_members"},
